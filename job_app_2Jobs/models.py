...
doc = """
Multiple players choose which of the available jobs to apply to, attempting to anti-coordinate with the other job applicants.
"""
...
class Constants(BaseConstants):
    ...
    treatment = 1 # Wages of (Job X, Job Y, Job Z) = ...(High, Medium, Low) in treatment 1; (High, Low, Low) in treatment 2; (Medium, Medium, Medium) in treatment 3.
#    wages = []
    wage_H = c(5)
    wage_M = c(3)
    wage_L = c(2)
    wages = [wage_H, wage_L]
#    wages_t1 = [wage_H, wage_L]

# Choose a name treatment:
    name_treatment = 'colors' # Name treatments: 'numbers', 'females' , 'males' , 'mixed' , 'colors'

    name_in_url = 'job_app_2Jobs'
    players_per_group = 2
    ...
    participation = c(5)
    instructions_template = 'job_app_2Jobs/Instructions_0.html'
    prize = c(0.5)
