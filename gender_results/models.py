...
#        self.ordering = p1.participant.vars['ordering']
        ...
        if self.round_number == 1:
            self.name = decider.participant.vars['name1']
        if self.round_number == 2:
...
# In each take round, use the other_person() function to get the gender of one's CP in that round. Save it to participant var _ that round, and use it here.
    def check_gender(self):
        decider = self.get_player_by_role('decider')
        receiver = self.get_player_by_role('receiver')
        if decider.genderCP1 == decider.participant.vars['gender_CP_1']:
            decider.guess1_is_correct = True
        else:
            decider.guess1_is_correct = False
        if receiver.genderCP1 == receiver.participant.vars['gender_CP_1']:
...
#        self.get_offer()
        ...
        receiver.payoff = self.participant.vars.get('offer',0)
    ...
    def get_offer(self):
        ...
        self.participant.vars['taken1'] = self.group.taken1
